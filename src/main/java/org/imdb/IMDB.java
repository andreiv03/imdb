...
public class IMDB {
  ...
  public static <T> ArrayList<T> readArrayOfObjectsFromJSON(String path,
	  TypeReference<ArrayList<T>> typeReference) {
  ...
  public void readFromDatabase() {
    ...
    this.productions = readArrayOfObjectsFromJSON(inputPath + "production.json",
	    productionsTypeReference);
    ...
    if (this.getUsers() != null && !this.getUsers().isEmpty()) {
      for (User user : this.getUsers()) {
        ...
        if (user.getProductionsContribution() != null && !user.getProductionsContribution()
          .isEmpty()) {
  ...
  public ArrayList<AbstractMap.SimpleEntry<String, Set<AccountType>>> instantiateActions() {
    ...
    for (int index = 0; index < actionsNames.size(); ++index)
      actions.add(new AbstractMap.SimpleEntry<>(actionsNames.get(index),
	      actionsAccountTypes.get(index)));
